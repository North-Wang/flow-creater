...
.response-event-node {
  @apply bg-white border-2 border-green-500 rounded-lg shadow-lg min-w-48;
  ...
  .node-header {
    @apply flex items-center justify-between p-3 bg-green-50 rounded-t-lg border-b border-green-200;

    .node-icon {
      @apply flex items-center justify-center w-8 h-8 bg-green-500 text-white rounded-full;
    }

    .node-title {
      @apply flex-1 ml-2 text-sm font-medium text-green-800;
    }
  ...
  .node-ports {
    ...
    .port {
      ...
      &.port-target {
        @apply absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-green-500;
      }
